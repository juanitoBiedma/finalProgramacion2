$(document).ready(function () {
  ...
  if (estaContratadoVigilante) {
    limitarRangoFechas();
    document
      .getElementById("txtEstaContratado")
      .addEventListener("change", function () {
        cargarSucursales();
        const sucursalvigilanteFields = document.getElementById(
          "sucursalvigilanteFields"
        );
...
function mostrarModal(mensaje, callback) {
  ...
  $("#alertModalRegistrarUsuario").off("hidden.bs.modal");
...
function validarFormularioVigilante() {
  let esValido = true;
  const camposRequeridos = document.querySelectorAll(
    "#formRegistrarVigilante [required]"
  );
...
async function crearUsuario() {
  ...
  if (datosUsuario.rolUsuario.id === "3") {
    ...
    datosVigilante = {
      ...
      estaContratadoVigilante:
        document.getElementById("txtEstaContratado").checked,
    };
    ...
    if (
      datosVigilante.edadVigilante < 18 ||
      datosVigilante.edadVigilante > 65
    ) {
    ...
    if (datosVigilante.estaContratadoVigilante) {
      ...
      if (
        fechaContrato < new Date("2024-01-01") ||
        ...
        if (requestVigilante.ok) {
          ...
          if (document.getElementById("txtEstaContratado").checked) {
            const idSucursal = document.getElementById(
              "txtSucursalVigilante"
            ).value;

            datosContrato = {
              fechaContrato: fechaContrato,
              tieneArmaContrato:
                document.getElementById("txtTieneArma").checked,
