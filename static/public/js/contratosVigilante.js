document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(`${window.env.BACKEND_URL}/auth/usuario-logueado`, {
    ...
    .then((data) => {
      ...
      if (data.rolUsuario.id !== 2) {
        document
          .getElementById("btnAsignarContrato")
  ...
  document
    .getElementById("confirmarAsignacionContratosVigilante")
    .addEventListener("click", asignarContrato);
  $("#contratos").DataTable();
});
...
async function asignarContrato() {
  ...
  try {
    const response = await fetch(
      `${window.env.BACKEND_URL}/api/vigilantes/${idVigilante}/asignarContrato`,
      {
        method: "POST",
        credentials: "include",
        headers: {
...
async function mostrarContratos() {
  ...
  try {
    ...
    contratos.forEach((contrato) => {
      ...
      const formattedDate = `${String(fechaContrato.getDate()).padStart(
        2,
        "0"
      )}/${String(fechaContrato.getMonth() + 1).padStart(
        2,
        "0"
      )}/${fechaContrato.getFullYear()}`;

      const contratoHTML = `<tr>
                <td>${contrato.idContrato}</td>
                <td>${formattedDate}</td>
                <td>${contrato.sucursal.entidad.nombreEntidad} - ${
        contrato.sucursal.nombreSucursal
      }</td>
    ...
    if (rolUsuarioLogueado === 2) {
      const actionsColumn = document.querySelectorAll(
        "#contratos th:nth-child(5), #contratos td:nth-child(5)"
      );
...
function confirmarEliminarContrato(idContrato) {
  ...
  document.getElementById("confirmarEliminarContratosVigilante").onclick =
    function () {
...
async function eliminarContrato(idContrato) {
  ...
  try {
    const response = await fetch(
      `${window.env.BACKEND_URL}/api/contratos/${idContrato}`,
      {
        method: "DELETE",
        credentials: "include",
        headers: {
    ...
    if (!response.ok) {
      ...
      if (!responseVigilante.ok) {
        ...
        if (!flagEstaContratado) {
          ...
          const responsePatchVigilante = await fetch(
            `${window.env.BACKEND_URL}/api/vigilantes/editarVigilante/${idVigilante}`,
            {
              method: "PATCH",
              credentials: "include",
              headers: {
          ...
          if (!responsePatchVigilante.ok) {
            throw new Error(
              "Ocurrió un error al intentar actualizar el vigilante."
            );
          }
        }
      }
...
function mostrarModal(mensaje, callback) {
  document.getElementById("alertModalBodyContratoVigilante").innerText =
    mensaje;
  $("#alertModalContratoVigilante").modal("show");

  $("#alertModalContratoVigilante").off("hidden.bs.modal");
