  ...
  try {
    ...
    delincuentes.forEach((delincuente) => {
      ...
      const accionesHTML =
        rolUsuarioLogueado === 1
          ? `${botonEliminar} ${botonEditarDelincuente} ${botonDelitos}`
          : "";
      ...
      if (delincuente.delitos.length > 0) {
        delincuente.delitos.forEach((delito) => {
          ...
          const sentencia = sentencias.find(
            (s) => s.delito.idDelito === delito.idDelito
          );
          const tiempoSentencia = sentencia
            ? `${sentencia.tiempoSentencia} años`
            : "sin condena";
      ...
      const delincuenteHTML = `<tr>
                    <td>${delincuente.idDelincuente}</td>
                    <td>${delincuente.nombreDelincuente} ${
        delincuente.apellidoDelincuente
      }</td>
                    <td>${
                      delincuente.banda
                        ? delincuente.banda.nombreBanda
                        : "Sin banda"
                    }</td>
                    <td>${delitosHTML}</td>
    ...
    if (rolUsuarioLogueado === 2) {
      ...
      const actionsColumn = document.querySelectorAll(
        "#delincuentes th:nth-child(5), #delincuentes td:nth-child(5)"
      );
...
function confirmarEliminarDelincuente(idDelincuente) {
  ...
  document.getElementById("confirmarEliminarDelincuente").onclick =
    function () {
...
async function eliminarDelincuente(idDelincuente) {
  try {
    ...
    if (responseDelincuente.ok) {
      const responseEliminarDelincuente = await fetch(
        `${window.env.BACKEND_URL}/api/delincuentes/${idDelincuente}`,
        {
          method: "DELETE",
          credentials: "include",
    ...
    mostrarModal(
      "Ocurrió un error al intentar eliminar al delincuente.",
      function () {
        window.location.href = "delincuentes.html";
      }
    );
