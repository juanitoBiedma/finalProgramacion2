  ...
  try {
    ...
    usuarios.forEach((usuario) => {
      ...
      const accionesHTML =
        rolUsuarioLogueado === 1
          ? `${botonEliminar} ${botonEditarUser} ${botonEditarPassword}`
          : "";
    ...
    if (rolUsuarioLogueado === 2) {
      ...
      const actionsColumn = document.querySelectorAll(
        "#usuarios th:nth-child(5), #usuarios td:nth-child(5)"
      );
...
async function eliminarUsuario(idUsuario) {
  try {
    const responseEliminarUsuario = await fetch(
      `${window.env.BACKEND_URL}/api/usuarios/eliminarCompleto/${idUsuario}`,
      {
        method: "DELETE",
        credentials: "include",
        headers: {
