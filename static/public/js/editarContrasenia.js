...
function mostrarModal(mensaje, callback) {
  document.getElementById("alertModalBodyEditarContrasenia").innerText =
    mensaje;
  $("#alertModalEditarContrasenia").modal("show");

  $("#alertModalEditarContrasenia").off("hidden.bs.modal");
...
function validarFormulario() {
  ...
  const camposRequeridos = document.querySelectorAll(
    "#formEditarContrasenia [required]"
  );
...
async function editarContrasenia() {
  ...
  const idUsuario = getQueryParam("idUsuario"); // Obtener el ID de la URL
  const antiguaContrasenia = document.getElementById(
    "txtEditarAntiguaPass"
  ).value;
  const nuevaContrasenia = document.getElementById("txtEditarNuevaPass").value;
  const repetirNuevaContrasenia = document.getElementById(
    "txtRepetirNuevaPass"
  ).value;
  ...
  try {
    const response = await fetch(
      `${window.env.BACKEND_URL}/api/usuarios/editarContrasenia/${idUsuario}`,
      {
        method: "PATCH",
        credentials: "include",
        headers: {
